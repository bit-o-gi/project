...
public interface AnRepository extends JpaRepository<Anniversary,Long> {
	...
	List<Anniversary> findAllByAntimeBetween(LocalDateTime startDate, LocalDateTime endDate);

	// NOTE: 특정 날짜 이후의 기념일 조회
	List<Anniversary> findAllByAntimeAfter(LocalDateTime date);

	// NOTE: 특정 날짜 이전의 기념일 조회
	List<Anniversary> findAllByAntimeBefore(LocalDateTime date);
